...
async function loadSite(url , websiteName, pages) {
  ...
  let finalUrl = url
  if (document.querySelector(`link[href*="./website/"]`)){
      document.querySelector(`link[href*="./website/"]`).remove()
  }  
  ...
  if (!document.querySelector(`link[href="./website/${websiteName}/${websiteName}.css"]`)) {
    const style = document.createElement('link');
    style.rel = 'stylesheet'
    style.href = `./website/${websiteName}/${websiteName}.css`;
...
const loadPages = () => {
  ...
  links.forEach(link => {
      ...
      loadSite(`./website/${link.dataset.link}/${link.dataset.link}.html` , link.dataset.link).catch(err => console.error(err));
    });
  });
...
const goBack = () => {
   if (document.querySelector(`link[href*="./website/"]`)) document.querySelector(`link[href*="./website/"]`).remove();
...
loadSite('./website/home/home.html', 'home')
